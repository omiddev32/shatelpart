...
use App\User\Entities\Permission;
...
trait HasRoleAndPermission
{
     ...
     *
     * @param array|\App\User\Entities\Role[]|\Illuminate\Database\Eloquent\Collection $roles
     *
     * @return array
     */
    ...
    public function rolePermissions()
    {
        ...
        if (config('roles.inheritance')) {
            ...
            return $permissionModel
                ...
                ->join($roleTable, $roleTable.'.id', '=', 'permission_role.role_id')
                ->whereIn($roleTable.'.id', $this->getRoles()->pluck('id')->toArray())
                ->groupBy(['permissions.group','permissions.id', 'permissions.name', 'permissions.slug', 'permissions.description', 'permissions.model', 'permissions.created_at', 'permissions.updated_at', 'permissions.deleted_at', 'pivot_created_at', 'pivot_updated_at']);
        }
    }
     ...
     * @return bool
     */
    public function allowed($providedPermission, Model $entity, $owner = true, $ownerColumn = 'admin_id')
    {
        if ($this->isPretendEnabled()) {
     ...
     *
     * @param array|\App\Role\Entities\Permission[]|\Illuminate\Database\Eloquent\Collection $permissions
     *
     * @return array
     */
    ...
    public function callMagic($method, $parameters)
    {
        if (starts_with($method, 'is')) {
            ...
            return $this->allowed(snake_case(substr($method, 7), config('roles.separator')), $parameters[0], (isset($parameters[1])) ? $parameters[1] : true, (isset($parameters[2])) ? $parameters[2] : 'admin_id');
        }
