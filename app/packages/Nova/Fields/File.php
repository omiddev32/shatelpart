...
class File extends Field implements StorableContract, DeletableContract, DownloadableContract
{
     ...
     * @param  \Laravel\Nova\Http\Requests\NovaRequest  $request
     * @param  string  $requestAttribute
     * @param  \Illuminate\Database\Eloquent\Model|\Laravel\Nova\Support\Fluent  $modelData
     * @param  string  $attribute
     * @return mixed
     */
    protected function fillAttribute(NovaRequest $request, $requestAttribute, $modelData, $attribute)
    {

        /* Updated for repateble */
        // Need update
        $model = $modelData;
        ...
        if (is_null($file = $this->retrieveFileFromRequest($request, $requestAttribute))) {
            ...
            if(isset(explode('.', $requestAttribute)[1])) {
                $key = explode('.', $requestAttribute)[1];
                if(isset($modelData[1][$key]) && $modelData[1][$key]['fields'] && isset($modelData[1][$key]['fields'][$attribute])) {
                    $model->{$attribute} = $modelData[1][$key]['fields'][$attribute];
                }
            }
