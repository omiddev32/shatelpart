...
use Laravel\Nova\Fields\{Field, File, FieldCollection, Image};
...
class JSON implements Preset
{
    ...
    public function set(
        ...
        $defaultModelData = $model->{$attribute};
        $model->setAttribute($attribute, null);


        $fieldCallbacks = collect($request->input($requestAttribute))
            ->map(function ($item, $blockKey) use ($request, $requestAttribute, $model, $attribute, $repeatables, $defaultModelData) {
                $data = new Fluent();
                ...
                $callbacks = $fields
                    ...
                    ->map(function (Field $field) use ($request, $requestAttribute, $data, $blockKey, $defaultModelData) {

                        $fieldData = $data;

                        if($field instanceof File || $field instanceof Image) {
                            $fieldData = [
                                $data, $defaultModelData
                            ];
                        }

                        return $field->fillInto($request, $fieldData, $field->attribute, "{$requestAttribute}.{$blockKey}.fields.{$field->attribute}");
                    })
