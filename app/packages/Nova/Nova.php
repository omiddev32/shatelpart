...
class Nova
{
     ...
     * @var bool
     */
    public static $withAuthentication = true;
     ...
     * @var bool
     */
    public static $withThemeSwitcher = false;
    ...
    public static function translations($translations)
    {
        if (is_string($translations)) {
            ...
            foreach ($translations as $translation) {
                if ($languages) {
                    $languages = json_encode(array_merge(json_decode($languages, true),json_decode(file_get_contents($translation), true)));
                }
                else {
                    $languages = file_get_contents($translation);
                }
            }
            static::$translations = array_merge(static::$translations, json_decode($languages , true));
            return new static;
        }
    ...
    public static function addMenuIcons($key, $name)
    {
        self::$menuIcons[$key] = $name;
    }
    ...
    public static function defaultMainMenu(Request $request)
    {
        return Menu::make(with(collect(static::availableTools($request)), function ($tools) use ($request) {
            ...
            foreach($tools as $tool) {
                $simular = $tool->menu($request);

                if($simular instanceof \Illuminate\Support\Collection) {
                    foreach($simular as $key => $item) {
                        if($item instanceof \Laravel\Nova\Menu\MenuGroup) {
                            foreach($item->items as $groupItem) {
                                $menus[$key][] = self::addMenuItemIconIfExists($groupItem, $groupItem->name);
                            }
                        } else {
                            $menus[$key][] = self::addMenuItemIconIfExists($item, $item->name);
                        }
                    }
                } else if($simular instanceof \Laravel\Nova\Menu\MenuSection) {
                    if($simular->path != '') {
                        $menus[$simular->name] = self::addMenuItemIconIfExists($simular, $simular->name);
                    } else {
                        $menus[$simular->mainGroup][] = self::addMenuItemIconIfExists($simular, $simular->name); 
                    }
                } else {
                   $menus[$simular->mainGroup][] = self::addMenuItemIconIfExists($simular, $simular->name); 
                }
            ...
            foreach($menus as $key => $group) {
                ...
                if($group instanceof \Laravel\Nova\Menu\MenuSection) {
                    $newMenus[] = $group;
                } else {
                    $newMenus[] = \Laravel\Nova\Menu\MenuGroup::make($key, $group)
                        ->icon(isset(self::$menuIcons[$key]) ? self::$menuIcons[$key] : null)
                        ->mainGroup($key)
                        ->collapsable();
                }
            ...
            return \Illuminate\Support\Collection::make($newMenus);

        })->filter()->values()->all());
    }


    protected static function addMenuItemIconIfExists($menu, $key)
    {
        if(isset(self::$menuIcons[$key])) {
            return $menu->icon(self::$menuIcons[$key]);
        }
    ...
    {
        return Blade::render('
            <p class="text-center">Powered by <a class="link-default" href="https://nova.laravel.com">Laravel Nova</a> · 
