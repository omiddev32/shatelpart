...
class FieldAttachmentController extends Controller
{
    ...
    public function store(NovaRequest $request)
    {
        ...
        $fieldAttribute = $request->field;
        if(str_contains($fieldAttribute, '.')) {
            $explode = explode('.', $request->field);
            if(in_array($explode[1], array_keys(config('translatable.locales')))) {
                $fieldAttribute = $explode[0];
            }
        }
        ...
        $field = $request->newResource()
                        ...
                        ->findFieldByAttribute($fieldAttribute, function () {
                            abort(404);
                        });
