...
class TranslatableFieldMixin
{
    ...
    public function translatable()
    {
        return function ($overrideLocales = [], $options = []) {
            $locales = ServiceProvider::getLocales($overrideLocales);
            ...
            $this->resolveUsing(function ($value, $resource, $attribute) use ($locales, $component, $originalResolveCallback, $options) {
                ...
                $attribute = ServiceProvider::normalizeAttribute($attribute);
                ...
                $prioritizeNovaLocale = isset($options['prioritizeNovaLocale'])
                    ...
                    : config('translatable.prioritize_nova_locale', true);

                $displayType = isset($options['displayType'])
                    ? $options['displayType']
                    : config('translatable.display_type', 'row');

                $fillOtherLocalesFrom = isset($options['fillOtherLocalesFrom'])
                    ? $options['fillOtherLocalesFrom']
                    : config('translatable.fill_other_locales_from', null);
            ...
            $this->fillUsing(function ($request, $model, $attribute, $requestAttribute) use ($locales) {
                ...
                /* Change for repeaters */
                if(str_contains($requestAttribute, 'fields') && count(explode('.', $requestAttribute)) > 1) {
                    $steps = explode('.', $requestAttribute);
                    $data = null;
                    $value = null;
                        ...
                        if(! $data) {
                            ...
                            if(isset($data[$step])) {
                                ...
                                foreach(array_keys($languages) as $lang) {
                                    if(isset($data["{$step}[{$lang}"])) {
                                        $value[$lang] = $data["{$step}[{$lang}"];
                                    }
                                }
                            }
                        }
                    ...
                    $realAttribute = ServiceProvider::normalizeAttribute($this->meta['translatable']['original_attribute'] ?? $attribute);
    ...
    public function rulesFor()
    {
        return function ($locales, $rules) {
            $setRule = function ($locale, $rules) {
                if (!in_array($locale, array_keys(ServiceProvider::getLocales()))) {
