...
import { loadNovaSettings } from '@packages/Settings/js/package'
...
const registerModules = (app) => {
  const modules = require.context('../../modules', true, /\/setup\.js$/);
  modules.keys().forEach(modulePath => {
    const moduleSetup = modules(modulePath);
    const moduleName = modulePath.match(/\.\/([^/]+)\/templates\/js\/setup\.js$/)[1];
    let funcName = `${moduleName}ModuleSetup`
    if(typeof moduleSetup[funcName] === "function") {
      moduleSetup[funcName](app)
    }
  });
}
...
class Nova {
   ...
   */
  isLtrText(text) {
    return text.match(/[a-z]+|^[\#\w]+$/gi) !== null;
  }
